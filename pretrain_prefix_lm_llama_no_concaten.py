...
def get_batch_pipe(data):
    ...
    # for llama tokenizer: >>> from transformers import LlamaTokenizer ; >>> tokenizer = LlamaTokenizer.from_pretrained('decapoda-research/llama-7b-hf')
    # >>> tokenizer.encode('\n\n### Response:\n') --> [1, 29871, 13, 13, 2277, 29937, 13291, 29901, 13]
    for i in range(len(tokens)):
        ...
        for j in partial_prefix_indice:
            if tokens[i][j-1]==29937:
                # use '\n' as the last token of prefix: 13291 --> Response
                # see `get_ltor_masks_and_position_ids`, <s> to `\n` is seen
                # loss: <s>->prefix#1 to `\n`->target#1 is mask (verified --> loss_mask[i][(tokens[i]==2).nonzero().squeeze(-1)[0]])
