...
class Encoder(object):
    ...
    def encode(self, json_line):
        prefix = Encoder.tokenizer.tokenize(
            "Below is a dialogue between the User and the AI Assistant developed by CloudWalk(云从科技). Write the Assistant's response that appropriately completes the User's request.\n")
        sep = Encoder.tokenizer.tokenize("###")
        user = Encoder.tokenizer.tokenize(" User: ")
        ai = Encoder.tokenizer.tokenize(" Assistant: ")
        data = json.loads(json_line)
        ids = {'text':[prefix], 'loss_mask':[[0] * len(prefix)]}
        ids['text'][0] += sep
        ids['loss_mask'][0] += [0]
...
def get_args():
    ...
    group.add_argument("--tokenizer-name-or-path", type=str, default='/disk2/mnt/megatron_deepspeed/data/tokenizer',
            ...
            for sample in all_samples:
                ...
                if len(text) + sample['len'][0] > args.cat_max_length:
                    if text:
                        ...
                        builders['text'].add_item(torch.IntTensor(sample['text'][0]))
                        builders['loss_mask'].add_item(torch.IntTensor(sample['loss_mask'][0]))
                        builders['att_mask'].add_item(torch.IntTensor(sample['att_mask'][0]))
                        lens[len(sample['text'][0])] += 1
                        text, loss_mask, att_mask = [], [], []
