        ...
        for j, file in enumerate(file_names):
            ...
            for key in keys:
                if 'query_key_value' in key:
                    ...
                    # check multiquery config
                    if args.num_key_value_heads:
                        assert args.num_attention_heads % args.num_key_value_heads == 0, 'num_attention_heads cannot be divided by num_key_value_heads'
                    ...
                    hidden_size_per_head = args.hidden_size // args.num_attention_heads
                    kv_hidden_size = hidden_size_per_head * args.num_key_value_heads if args.num_key_value_heads else args.hidden_size
                    qkv = tensors.pop(key)
                    q = torch.zeros((args.hidden_size, args.hidden_size))
                    k = torch.zeros((kv_hidden_size, args.hidden_size))
                    v = torch.zeros((kv_hidden_size, args.hidden_size))
                    query_head_per_group = args.num_attention_heads // args.num_key_value_heads if args.num_key_value_heads else 1
                    offset = 0
                    offsetq = 0
                    offsetqkv = 0
                    for _ in range(args.num_attention_heads):
                        q[offsetq:offsetq + query_head_per_group * hidden_size_per_head] = \
                            qkv[offsetqkv: offsetqkv + query_head_per_group * hidden_size_per_head]
                        k[offset:offset + hidden_size_per_head] = \
                            qkv[offsetqkv + query_head_per_group * hidden_size_per_head:offsetqkv + (query_head_per_group + 1) * hidden_size_per_head]
                        v[offset:offset + hidden_size_per_head] = \
                            qkv[offsetqkv + (query_head_per_group + 1) * hidden_size_per_head:offsetqkv + (query_head_per_group + 2) * hidden_size_per_head]
                        offset += hidden_size_per_head
                        offsetq += query_head_per_group * hidden_size_per_head
                        offsetqkv += (query_head_per_group + 2) * hidden_size_per_head
