...
def reshape_embedding_ckpt(path, path_save, resize_shape):
    ...
    for idx, ckpt in enumerate(ckpt_list):
        if idx == 0 or idx == len(ckpt_list) - 1:
            ...
            word_embeddings_avg = word_embedding[:72964].mean(dim=0, keepdim=True)
            new_word_embedding = torch.zeros(resize_shape, word_embedding.shape[1], dtype=word_embedding.dtype)
            new_word_embedding[:72964] = word_embedding
            new_word_embedding[72964:] = word_embeddings_avg
...
if __name__=='__main__':
    path = '/workspace/yanc/yanchao/llama2/llama-cw-204B-meg-ds/global_step0/'
    path_save = '/workspace/yanc/yanchao/llama2/llama-cw-204B-meg-ds/global_step1'
    resize_shape = 73088
    ...
    assert resize_shape % 128 == 0 and resize_shape > 72964, 'resize_shape need to be devisible by 128 and larger than origin llama embedding shape'
