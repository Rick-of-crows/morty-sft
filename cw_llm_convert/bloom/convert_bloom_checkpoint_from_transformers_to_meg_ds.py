...
#adapte for xuanyuan & chatbloom
NON_TRANSFORMERS_BLOCK_WEIGHTS = {
    "word_embeddings.weight": "transformer.word_embeddings.weight",
    "word_embeddings.norm.weight": "transformer.word_embeddings_layernorm.weight",
    "word_embeddings.norm.bias": "transformer.word_embeddings_layernorm.bias",
...
def get_transformers_weight_names(meg_ds_weight: str, layer_id: Optional[int]) -> List[str]:
    if layer_id is None:
        ...
        return [f"transformer.h.{layer_id}.{tfrs_block_name}" for tfrs_block_name in TRANSFORMERS_BLOCK_WEIGHTS[meg_ds_weight]]
        # return [f"h.{layer_id}.{tfrs_block_name}" for tfrs_block_name in TRANSFORMERS_BLOCK_WEIGHTS[meg_ds_weight]]
...
def merge_layers(layers, num_heads: int, hidden_size: int):
    ...
    #     return layers[0]
    # else:
    assert len(layers) == 1, 'length of layers in not 1'
...
def convert_opt_checkpoint_to_megatron(
    ...
    with open(os.path.join(opt_checkpoint_path, "config.json"), "r") as fi:
        config = json.load(fi)
    total_amount_of_layers = config["n_layer"]
