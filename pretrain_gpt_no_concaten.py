...
def model_provider(pre_process=True, post_process=True):
    ...
    with deepspeed.zero.Init(data_parallel_group=mpu.get_data_parallel_group(),
                             ...
                             mpu=mpu):
        if args.deepspeed:
            # whether to use the attention mask generate from `get_batch_pipe`, if True, then set attention mask None
...
def get_batch_pipe(data):
    ...
    position_ids = None
    if 'loss_mask' not in keys or 'att_mask' not in keys:
        tokenizer = get_tokenizer()
        # Get the masks and position ids.
        attention_mask, loss_mask, position_ids = get_ltor_masks_and_position_ids(
            tokens,
            tokenizer.eod,
            args.pad_id,  # add for padding
            args.reset_position_ids,
            args.reset_attention_mask,
            args.eod_mask_loss,
            prefix_indices=None,
        ...
        # mask prefix loss, but no prefix attention mask.
        # experiment has done. drop this case. follow the bloom intent: if there is no prefix mask reset, neither loss on target only.
        if args.loss_on_targets_only:
            ...
            for i in range(len(tokens)):
                ...
                if len(_partial_prefix_indice) == 0:
                    ...
                    # prefix_indices[b] is the index of first token of target.
                loss_mask[i, :_partial_prefix_indice[0]] = 0.0

        # padding mask for no concatenate training.
        for b in range(len(tokens)):
            eod_indices = (tokens[b] == tokenizer.eod).nonzero().squeeze(-1)
            if len(eod_indices) == 0:
                eod_index = args.seq_length
            else:
                eod_index = eod_indices[0]
            # dont use `eod` to predict: because the next token of `eod` is `below` (due to no concaten tokens building) -wsf 20230504
            loss_mask[b, eod_index:] = 0.0
    ...
    if 'att_mask' in keys:
        att_mask = data_b['att_mask'].long()
        att_mask = att_mask[:, :-1].contiguous()

        bts, seq_len = att_mask.shape
        x = torch.arange(seq_len).to(att_mask.device)
        d = x - att_mask
        attention_mask = x.expand((bts, seq_len, seq_len))
        attention_mask = (attention_mask < d.unsqueeze(-1)) + (attention_mask > x.unsqueeze(-1))
        attention_mask = attention_mask.unsqueeze(1) # [bts, 1, seq_len, seq_len]

    if position_ids is None:
        position_ids = torch.arange(seq_len, dtype=torch.long, device=att_mask.device)
