...
def _build_train_valid_test_datasets(data_prefix, data_impl, splits_string,
    ...
    indexed_loss_masks = None
    if args.input_loss_mask:
        indexed_loss_masks = get_indexed_dataset_(data_prefix.replace('text', 'loss_mask'),
                                           data_impl,
                                           skip_warmup)
        assert indexed_dataset.sizes.shape == indexed_loss_masks.sizes.shape

    indexed_att_masks = None
    if args.input_att_mask:
        indexed_att_masks = get_indexed_dataset_(data_prefix.replace('text', 'att_mask'),
                                              data_impl,
                                              skip_warmup)
        assert indexed_dataset.sizes.shape == indexed_att_masks.sizes.shape
    ...
    def build_dataset(index, name):
        ...
        if splits[index + 1] > splits[index]:
            documents = np.arange(start=splits[index], stop=splits[index + 1],
                                  step=1, dtype=np.int32)
            dataset = GPTDataset(name, data_prefix,
                                  documents, indexed_dataset, indexed_loss_masks, indexed_att_masks,
                                  train_valid_test_num_samples[index],
                                  seq_length, seed)
        return dataset
...
class GPTDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name, data_prefix, documents, indexed_dataset, indexed_loss_masks, indexed_att_masks,
                 num_samples, seq_length, seed):

        args = get_args()
        self.pad_id = args.pad_id
        self.use_dynamic_padding = args.use_dynamic_padding
        #assert self.pad_id == 3
        self.name = name
        self.indexed_dataset = indexed_dataset

        # importing loss_mask or att_mask is disabled in dynamic padding mode now (Todo.)
        if args.use_dynamic_padding:
            print('Will NOT import loss_mask and att_mask in dynamic padding mode.')
            self.indexed_loss_masks = None
            self.indexed_att_masks = None
        else:
            self.indexed_loss_masks = indexed_loss_masks
            self.indexed_att_masks = indexed_att_masks
    ...
    def __getitem__(self, idx):
        ...
        if doc_index_f == doc_index_l:
            ...
            if self.indexed_loss_masks is not None:
                loss_mask = self.indexed_loss_masks.get(self.doc_idx[doc_index_f],
                                                        offset=offset_f,
                                                        length=offset_l - offset_f + 1)

            if self.indexed_att_masks is not None:
                att_mask = self.indexed_att_masks.get(self.doc_idx[doc_index_f],
                                                      offset=offset_f,
                                                      length=offset_l - offset_f + 1)
        else:
            ...
            if self.indexed_loss_masks is not None:
                loss_mask_list = [self.indexed_loss_masks.get(self.doc_idx[doc_index_f],
                                                              offset=offset_f)]
                # Loop over all in between documents and add the entire document.
                for i in range(doc_index_f + 1, doc_index_l):
                    loss_mask_list.append(self.indexed_loss_masks.get(self.doc_idx[i]))

                loss_mask_list.append(self.indexed_loss_masks.get(
                    self.doc_idx[doc_index_l],
                    length=offset_l))
                loss_mask = np.concatenate(loss_mask_list)

            if self.indexed_att_masks is not None:
                att_mask_list = [self.indexed_att_masks.get(self.doc_idx[doc_index_f],
                                                            offset=offset_f)]
                # Loop over all in between documents and add the entire document.
                for i in range(doc_index_f + 1, doc_index_l):
                    att_mask_list.append(self.indexed_att_masks.get(self.doc_idx[i]))

                att_mask_list.append(self.indexed_att_masks.get(
                    self.doc_idx[doc_index_l],
                    length=offset_l))
                att_mask = np.concatenate(att_mask_list)
        ...
        if not self.use_dynamic_padding:
            if len(sample) >= self.seq_length:
                ...
                sample = np.concatenate([sample, np.array([self.pad_id] * (self.seq_length - len(sample)))])
                if self.indexed_loss_masks is not None:
                    loss_mask = np.concatenate([loss_mask, np.array([0] * (self.seq_length - len(loss_mask)))])
                if self.indexed_att_masks is not None:
                    att_mask = np.concatenate([att_mask, np.array([0] * (self.seq_length - len(att_mask)))])

        ret = {'text': np.array(sample, dtype=np.int64)}
        if self.indexed_loss_masks is not None:
            ret['loss_mask'] = np.array(loss_mask, dtype=np.int64)
        if self.indexed_att_masks is not None:
            ret['att_mask'] = np.array(att_mask, dtype=np.int64)
        return ret
...
def _build_index_mappings(name, data_prefix, documents, sizes,
    ...
    num_epochs = (num_samples - 1) // len(documents) + 1
    if torch.distributed.get_rank() == 0:
        print('documents: {}, total samples: {}, need {} epoch to trian.'.format(len(documents), num_samples, num_epochs))
    # rng state
    ...
    if torch.distributed.get_rank() == 0:
        if (not os.path.isfile(doc_idx_filename)) or \
           ...
           (not os.path.isfile(shuffle_idx_filename)):
            ...
            # use one seqence per sample, don't concatenate the sequence as pretrain
            sample_idx = _build_sample_idx_prefix(sizes, doc_idx, seq_length,
            ...
            shuffle_idx = np.arange(start=0, stop=total_size, step=1, dtype=dtype_)
