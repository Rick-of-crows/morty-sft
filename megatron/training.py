...
def pretrain(train_valid_test_dataset_provider,
    ...
    print_datetime('after model, optimizer, and learning rate '
                   'scheduler are built')
    '''
    # calculate per GPU parameters load
    def param_size(parameter):
        return parameter.ds_numel if hasattr(parameter, 'ds_id') else parameter.nelement()
    if args.rank == 0:
        param_list = [p for model_module in model for p in model_module.parameters()]
        approx_parameters_in_billions = sum(dict((p.data_ptr(), param_size(p)) for p in param_list).values()) / (1e9)
        print("rank 0 number of parameters: {}B".format(approx_parameters_in_billions))
        
        optim_para_list = [int(group['params'][0].shape[0]) for group in optimizer.param_groups]
        approx_optim_parameters_in_billions = sum(optim_para_list) / (1e9)
        print("rank 0 number of optimizer parameters: {}B".format(approx_optim_parameters_in_billions))

        # adamW
        # exp_avg_sq_list = [int(state['exp_avg_sq'].shape[0]) for state in optimizer.optimizer.state_dict()['state'].values()]
        # lion and adamW
        exp_avg_list = [int(state['exp_avg'].shape[0]) for state in optimizer.optimizer.state_dict()['state'].values()]
        approx_exp_parameters_in_billions = sum(exp_avg_list) / (1e9)
        print("rank 0 number of optimizer exp_avg_sq parameters: {}B".format(approx_exp_parameters_in_billions))

    # if args.rank == 0:
    layer=[(name, para) for name, para in model[0].named_parameters()]
    name, para = layer[5]
        # if 'input_layernorm' in name:
    print("#### print tag: {} : {}, local rank: {}".format(name, para, args.rank))
    '''
...
def train_step(forward_step_func, data_iterator,
    ...
    print("{} finish backward all-reduce and optimizer update", args.rank)
    timers.log(['backward-embedding-all-reduce', 'optimizer'])
    ...
    while iteration < args.train_iters:
        ...
        if args.curriculum_learning and \
            ...
            args.curriculum_seqlen = args.curriculum_scheduler.update_difficulty( \
                    args.iteration + 1)
        '''
        #if args.rank == 0 or args.rank == 7 or args.rank == 17 or args.rank == 27 or args.rank == 37:
        if args.rank == 0:
            print("### device: ", args.rank)
            layer=[(name, para) for name, para in model[0].named_parameters()]
            for name, para in layer:
                #if 'input_layernorm' in name:
                if 'ln_attn' in name:
                    print("#### print tag layernorm : {}".format(para))
        '''
