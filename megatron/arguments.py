...
def parse_args(extra_args_provider=None, defaults={},
    ...
    # CHECKPOINT_PATH="checkpoints/bloom-560m"
    ...
    # args.train_iters=500000
    # args.lr_decay_iters=320000
    # args.save=CHECKPOINT_PATH
    # args.load=CHECKPOINT_PATH
    # args.data_path=DATA_PATH
    # args.vocab_file="data/gpt2-vocab.json"
    # args.merge_file="data/gpt2-merges.txt"
    # args.data_impl="mmap"
    # args.split=949,50,1
    ...
    # args.micro_batch_size=$MICRO_BATCH
    # args.global_batch_size=$GLOBAL_BATCH
    ...
    # args.init_method_std=0.006
    # args.${DTYPE}=\
    # args.checkpoint_activations=\
    # args.exit_interval=10000
    # args.tensorboard_dir=$LOG_DIR

    # # run_fp_16_fast
    # CHECKPOINT_PATH="checkpoints/gpt2-345m"
    ...
    # args.adam_eps=1e-8
    # args.lr=1e-4
    # args.lr_warmup_samples=5
    # args.min_lr=1e-6
    # args.lr_decay_style="cosine"
    # args.lr_decay_samples=12
    # args.clip_grad=1.0
    # args.weight_decay=1e-1
    
    # args.embed_layernorm=True
    # args.fp16=True
    # args.partition_activations=True
    # args.seed=42
    # args.vocab_file=VOCAB_FILE
    # args.merge_file=MERGE_FILE

    # args.exit_interval=100
    ...
    # args.load=CHECKPOINT_PATH
    # args.data_path=DATA_PATH
    # args.tensorboard_dir=TENSORBOARD_PATH
    # args.tensorboard_queue_size=5
    # args.log_timers_to_tensorboard=True
    # args.log_batch_size_to_tensorboard=True
    # args.log_validation_ppl_to_tensorboard=True
    # # args.kill_switch_path=/tmp/kill_switch
    ...
    # # debug run bloom-560m
    # CHECKPOINT_PATH="checkpoints/bloom-560m"
    # SAVE_CHECKPOINT_PATH="output_dir/checkpoints/debug/bloom-560m"
    # DATA_PATH=["data/alpaca-data/my-gpt2_text_document"]    # 使用sh脚本启动和debug启动可能有所不同，debug设置需要设置成列表（sh脚本则依然使用字符串？）
    # # DATA_PATH=["data/alpaca-data/my-gpt2_output_document", "data/alpaca-data/my-gpt2_input_document", "data/alpaca-data/my-gpt2_instruction_document"]    # 使用sh脚本启动和debug启动可能有所不同，debug设置需要设置成列表（sh脚本则依然使用字符串？）
    # TENSORBOARD_PATH="output_dir/tensorboard/debug"
    ...
    # args.global_batch_size=GLOBAL_BATCH_SIZE 
    # args.train_samples=TRAIN_SAMPLES
    # args.pp_partition_method="type:transformer|embedding"
    # args.tokenizer_type="PretrainedFromHF"
    # args.tokenizer_name_or_path="data/tokenizer/tokenizer"
    # args.loss_scale=12
    # args.clip_grad=1.0
    # args.fp16=True
    # args.embed_layernorm=True       # 控制embedding层是否加LayerNorm
    # args.init_method_std=0.0048     # 正态分布初始化参数
    # args.position_embedding_type=PositionEmbeddingType.alibi
    # args.pad_vocab_size_to=250880
    ...
    # args.save=SAVE_CHECKPOINT_PATH
    # args.load=CHECKPOINT_PATH
    # args.data_path=DATA_PATH
    # args.data_impl="mmap"
    # args.split="949,50,1"
    ...
    # # added
    # args.no_load_optim=True
    # # args.universal_checkpoint=True      # 需要模型文件格式类似：zero/tied_modules.embed.word_embeddings.weight/fp32.pt
    # args.finetune=True                  # 控制zero-optim加载，cosumed_train_samples加载等
    # args.override_lr_scheduler=True
    
    # # for prefix run
    # args.reset_attention_mask=True
    # args.loss_on_targets_only=True
    ...
    if args.glu_activation is not None and args.hidden_activation is not None:
        raise ValueError("only one of glu-activation or hidden-activation can be used")
...
def _add_network_size_args(parser):
    ...
    group.add_argument('--parallel-attn', action='store_true',
                       help='support for falcon, parallel the self_attn with MLP')
    ...
    group.add_argument('--hidden-activation', type=str,
                    help='hidden activations to use, compatible for transformers activation implementation.'
                    )
    
    group.add_argument('--n-head-kv', type=int, default=None,
                    help='support for falcon multi-query, number of key/value heads.')
...
def _add_checkpointing_args(parser):
    ...
    group.add_argument('--llama-hf-ckpt', type=str, default=None,
                    help='path to llama huggingface checkpoint for convert.')
    group.add_argument('--bloom-hf-ckpt', type=str, default=None,
                    help='path to bloom huggingface checkpoint for convert.')
    return parser
...
def _add_mixed_precision_args(parser):
    ...
    # add @yanc support interpolation in rotary position embedding
    group.add_argument('--interpolation', type=int, default=None, help="Use interpolation in RotaryEmbedding")
...
def _add_data_args(parser):
    ...
    group.add_argument('--data-path', nargs='*', default=None,
                       ...
                       'dataset2-path ...')
    group.add_argument('--input_loss_mask', action='store_true',
                       help='If true, loss_mask will be imported from training dataset.')
    group.add_argument('--input_att_mask', action='store_true',
                       help='If true, att_mask will be imported from training dataset.')
    ...
    group.add_argument("--pad-id", type=int, default=None, help="corresponding to tokenizer padding id")
    group.add_argument("--no-use-padding", action='store_false', 
                       help="whether to do padding during get_batch in data sampler", 
                       dest="use_padding")
    group.add_argument("--use-dynamic-padding", action='store_true', 
                    help="whether to do use dynamic padding durning training, need modify deepspeed/runtime/pipe/engine.py to reset activation")
