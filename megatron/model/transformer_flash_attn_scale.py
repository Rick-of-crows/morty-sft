...
# from flash_attn.flash_attn_triton import FlashAttnQKVPackedFunc
# from .flash_attn_triton import FlashAttnQKVPackedFunc
# def flash_attn_qkvpacked_func(qkv, bias=None, causal=True, softmax_scale=None):
#     return FlashAttnQKVPackedFunc.apply(qkv, bias, causal, softmax_scale)

# from .flash_attn_triton_deprecated_temp import FlashAttnQKVPackedFunc
# from .flash_attn_triton_dropout_temp import FlashAttnQKVPackedFunc
from .flash_attn_triton_dropout_scale import FlashAttnQKVPackedFunc
def flash_attn_qkvpacked_func(qkv, bias=None, causal=True, softmax_scale=None, dropout_p=0, scale=1):
    return FlashAttnQKVPackedFunc.apply(qkv, bias, causal, softmax_scale, dropout_p, scale)
...
class ParallelAttention(MegatronModule):
    ...
    def forward(self, hidden_states, attention_mask, layer_past=None,
        ...
        if self.attention_type == AttnType.self_attn:
            ...
            # notice: np: head-num / tensor-model-pipeline-parallelism
            ...
            if self.use_flash_attn:
                ...
                # args = get_args()
                nheads = self.num_attention_heads_per_partition
                
                batch_size = hidden_states.shape[1]
                seqlen = hidden_states.shape[0]

                # cannot support bias(like alibi) now
                # mixed_x_layer=rearrange(mixed_x_layer.transpose(0, 1), 'b sq (np three hn) -> (b sq) three hn np', three=3, hn=nheads)
                # cu_seqlens = torch.arange(0, (batch_size + 1) * seqlen, step=seqlen, dtype=torch.int32,
                #             device=mixed_x_layer.device)
                
                # from flash_attn.flash_attn_interface import flash_attn_unpadded_qkvpacked_func

                # # [(b s) hn np]
                # output = flash_attn_unpadded_qkvpacked_func(
                #     mixed_x_layer, cu_seqlens, seqlen, args.attention_dropout,
                #     softmax_scale=None, causal=True
                # )

                # triton implimation
                mixed_x_layer=rearrange(mixed_x_layer.transpose(0, 1), 'b sq (np three hn) -> b sq three np hn', b=batch_size, sq=seqlen, three=3, np=nheads)

                # [b sq np hn]
                # output=flash_attn_qkvpacked_func(mixed_x_layer, bias=None, causal=True, softmax_scale=None)
                
                # [b, np, sq, sk]
                output_size = (mixed_x_layer.size(0),
                    mixed_x_layer.size(3),
                ...
                matmul_result = rearrange(matmul_result, '(b np) ... -> b np ...', b=batch_size)
                
                # ignore beta and alpha in origin bloom implementation, because
                # in the case of `self.apply_query_key_layer_scaling=True`, dont need to pass softmax_scale, equal to default implementation
                # so only support the case `self.apply_query_key_layer_scaling=True`
                # output: [b, sq, np, hn]
                scale = self.layer_number if self.apply_query_key_layer_scaling else 1.0
                output=flash_attn_qkvpacked_func(mixed_x_layer, bias=matmul_result, causal=True, softmax_scale=None, dropout_p=self.attention_dropout, scale=scale)

                # [b, sq, np, hn] --> [sq, b, hp]
                output = rearrange(output, 'b sq np hn -> sq b (np hn)')

                # =================
                # Output. [sq, b, h]
                # =================

                # wil do all reduce: hp --> h
        ...
        if alibi is None:
            ...
            # origin matmul_result shape is [b * np, 1, sq]
            # baddbmm: beta * input + alpha * (batch1 * batch2), second term shape is [b * np, sq, sk]
            # so origin matmul_result need to boardcast ot [b * np , sq ,sk]
            # [b * np, sq, sk]
            matmul_result = torch.baddbmm(
                matmul_result,
                query_layer.transpose(0, 1),  # [b * np, sq, hn]
