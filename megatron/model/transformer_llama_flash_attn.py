...
from .positional_embeddings_llama import RotaryEmbedding, apply_rotary_pos_emb_torch, apply_rotary_pos_emb
...
class ParallelMLP(MegatronModule):
    ...
    def __init__(self, init_method, output_layer_init_method):
        ...
        self.act_fn = ACT2FN[args.hidden_activation]


    def forward(self, hidden_states):

        # [s, b, 4hp]
        intermediate_parallel, bias_parallel = self.up_proj(hidden_states)
        intermediate_gate_parallel, bias_gate_parallel = self.gate_proj(hidden_states)
        ...
        intermediate_parallel = self.act_fn(intermediate_gate_parallel) * intermediate_parallel
        # [s, b, h]
...
class ParallelAttention(MegatronModule):
    ...
    def __init__(self, init_method,
        ...
        if self.position_embedding_type == PositionEmbeddingType.rotary:
            self.rotary_emb = RotaryEmbedding(self.hidden_size_per_attention_head, max_position_embeddings=args.seq_length, precision=args.params_dtype, interpolation=args.interpolation)
    ...
    def forward(self, hidden_states, attention_mask, layer_past=None,
        ...
        if self.attention_type == AttnType.self_attn:
            ...
            #     mixed_x_layer = rearrange(mixed_x_layer.transpose(0, 1), 'b sq (np three hn) -> b sq three np hn', 
            #                         b=hidden_states.shape[1], sq=hidden_states.shape[0], three=3, np=self.num_attention_heads_per_partition)
            #     output = flash_attn_qkvpacked_func(mixed_x_layer, dropout_p=0, softmax_scale=None, causal=True)
            #     # [b, sq, np, hn] --> [sq, b, hp]
        ...
        if alibi is None:
            # bias need to be zero vertor in llama.
            matmul_result = torch.empty(
            # torch.empty will init a non-zero tensor
        ...
        if self.use_flash_attn:
            ...
            query_layer = rearrange(query_layer, 'sq (b np) hn -> b sq np hn', b=hidden_states.shape[1])
            key_layer = rearrange(key_layer, 'sk (b np) hn -> b sk np hn', b=hidden_states.shape[1])
            # value layer: [sq, b, np, hn] -> [b, sq, np, hn]
            output = flash_attn_func(query_layer, key_layer, value_layer.transpose(0, 1), dropout_p=0, softmax_scale=None, causal=True)
            # [b, sq, np, hn] --> [sq, b, hp]
            output = rearrange(output, 'b sq np hn -> sq b (np hn)')
