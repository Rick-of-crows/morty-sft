    ...
    def __init__(self, init_method,
        ...
        if attention_type == AttnType.self_attn:
            self.query_key_value = mpu.ColumnParallelLinear(
                args.hidden_size,
                3 * projection_size if not args.multi_query else (projection_size + 2 * args.kv_channels),
                bias=False,
                gather_output=False,
                init_method=init_method,
                skip_bias_add=True)
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def __init__(self, init_method, output_layer_init_method,
        ...
        self.post_attention_layernorm = SyncLayerNorm(
            args.hidden_size,
            eps=args.layernorm_epsilon)
        ...
        if self.layer_type == LayerType.decoder:
            ...
            # Layernorm on the attention output.
            self.post_inter_attention_layernorm = SyncLayerNorm(
                args.hidden_size,
                eps=args.layernorm_epsilon)
    ...
    def forward(self, hidden_states, attention_mask,
        ...
        bias_dropout_add_func = bias_dropout_add(self.training)
