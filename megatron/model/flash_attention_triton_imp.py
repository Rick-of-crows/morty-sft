...
def _get_seed_offset(off_hb, start_m, start_n):
    return off_hb * 32769 + start_m* 32767  + start_n + 1
...
def _fwd_kernel(
    ...
    debug_stride_b, debug_stride_h, debug_stride_s,
    ...
    if DEBUG:
        debug_ptr = debug_tensor + off_b * debug_stride_b +  off_h * debug_stride_h + (offs_m[:,None] * debug_stride_s + offs_n[None,:]) 
    if BIAS_TYPE == 'vector':
    ...
    if HAS_DROPOUT > 0:
        ...
        rint = tl.randint(seed, offset)
        if not SLOW_RANDOM:
            ra = rint# * _get_seed_offset(off_hb, start_m * BLOCK_M, start_n)
            r = tl.uint32_to_uniform_float(ra)
    ...
    for start_n in range(0, end_n, BLOCK_N):
        ...
        if HAS_DROPOUT:
            ...
            if DEBUG:
                r_copy = tl.reshape(r, (BLOCK_M, BLOCK_N))
                tl.store(debug_ptr + start_n , r_copy)
        # scale acc_o
...
def _bwd_kernel_one_col_block(
    ...
    DO, DQ, DK, DV,
    LSE, D,
    softmax_scale, seed, pro, co, debug_tensor, off_hb,
    ...
    DEBUG: tl.constexpr, SLOW_RANDOM: tl.constexpr,
):
    ...
    if HAS_DROPOUT: 
        ...
        rint = tl.randint(seed, offset)
        if not SLOW_RANDOM:
            ra = rint# * _get_seed_offset(off_hb, start_m, start_n*BLOCK_N)
            r = tl.uint32_to_uniform_float(ra)
            ...
            p_scale = 1. / (1. - pro)
    if DEBUG:
        debug_ptr = debug_tensor + (offs_m[:,None] * debug_stride_s + offs_n[None,:]) 
    # loop over rows
    ...
    for start_m in range(begin_m, num_block_m * BLOCK_M, BLOCK_M):
        ...
        if HAS_DROPOUT:
            ...
            p *= tl.where(keep, 1.0, -1.0)
            #p = tl.where(keep, (p*p_scale), 0.)
            if DEBUG:
                r_copy = tl.reshape(r, (BLOCK_M, BLOCK_N))
                tl.store(debug_ptr + start_m * debug_stride_s , r_copy)
        ####################
        ...
        if HAS_DROPOUT:
            p_dropped = tl.where(keep, p * (1.0 / (1.0 - pro)), 0.0)
        ...
        if HAS_DROPOUT:
            ds = softmax_scale * p * tl.where(p > 0.0, (dp * (1.0 / (1.0 - pro))) - Di[:, None], Di[:, None])
            ds = ds.to(q.dtype)
        else:
...
def _bwd_kernel(
    ...
    DO, DQ, DK, DV,
    LSE, D,
    softmax_scale, seed, pro,co , debug_tensor,
    stride_qb, stride_qh, stride_qm,
    stride_kb, stride_kh, stride_kn,
    stride_vb, stride_vh, stride_vn,
    stride_bb, stride_bh, stride_bm,
    ...
    if DEBUG:
        debug_tensor += off_b * debug_stride_b + off_h * debug_stride_h
    if BIAS_TYPE != 'none':
    ...
    if not SEQUENCE_PARALLEL:
        ...
        for start_n in range(0, num_block_n):
            _bwd_kernel_one_col_block(
                ...
                DEBUG=DEBUG, SLOW_RANDOM=SLOW_RANDOM,
            )
    else:
...
def _flash_attn_forward(q, k, v, bias=None, causal=False, softmax_scale=None, p=0, co=1):
    # shape constraints
    ...
    seed = torch.randint(65536, (1,), device=q.device, dtype=torch.int32).item()

    debug_tensor = None
    debug_stride_b = debug_stride_h = debug_stride_s = 0
    SLOW_RANDOM = (os.environ.get("SLOW_RANDOM") == '1')
    DEBUG = (os.environ.get("DEBUG_FLASH_ATT") == '1')
    if DEBUG:
        debug_tensor = torch.zeros((batch, seqlen_q, nheads, seqlen_k), device=q.device, dtype=q.dtype)
        debug_stride_b = debug_tensor.stride(0)
        debug_stride_h = debug_tensor.stride(2)
        debug_stride_s = debug_tensor.stride(1)
    ...
    _fwd_kernel[grid](
        q, k, v, bias, o,
        lse, tmp,
        softmax_scale, seed, p,co, debug_tensor,
        ...
        *bias_strides,
        ...
        HAS_DROPOUT=p>0,DEBUG=DEBUG,SLOW_RANDOM=SLOW_RANDOM,
        num_warps=num_warps,
        num_stages=1,
    )
    if DEBUG:
        array = debug_tensor.float().cpu().numpy()
        np.save('fw_debug.npy', array)
    return o, lse, softmax_scale, seed  # softmax_scale could have been updated


def _flash_attn_backward(do, q, k, v, o, lse, dq, dk, dv, bias=None, causal=False, softmax_scale=None, seed=None, pro=0, co=1):
...
class FlashAttnFunc(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, bias=None, causal=False, softmax_scale=None, p=0, co=1):
        """
        ...
        o, lse, ctx.softmax_scale, ctx.seed = _flash_attn_forward(
            q, k, v, bias=bias, causal=causal, softmax_scale=softmax_scale, p=p, co=co
        )
    ...
    def backward(ctx, do):
        ...
        with torch.inference_mode():
            ...
            _flash_attn_backward(do, q, k, v, o, lse, dq, dk, dv,
                                 bias=bias, causal=ctx.causal, softmax_scale=ctx.softmax_scale, seed=ctx.seed, pro=ctx.p, co=ctx.co)
