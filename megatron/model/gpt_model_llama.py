...
from .transformer_llama_flash_attn import LlamaRMSNorm
# for llama-70b multiquery
#from .transformer_llama_multiquery import LlamaRMSNorm
from .transformer_llama_flash_attn import ParallelTransformerLayerPipe
# for llama-70b multiquery
#from .transformer_llama_multiquery import ParallelTransformerLayerPipe
...
class FinalLMHead(MegatronModule):
    """A lm_head layer.
    
    the lm_head weights is different with word_embeddings weights in Llama
    input should be [b, s, h]
    """
    
    def __init__(self, parallel_output, init_method):
        args = get_args()

        super(FinalLMHead, self).__init__()
        
        # the layer name must be `final_linear` to support quantize in 
        # `deepspeed.runtime.state_dict_factory.split_state_dict` when loading weights with PP > 1
    ...
    def forward(self, hidden_states):
        ...
        lm_head_output = self.final_linear(hidden_states)
        
        # mpu.ColumnParallelLinear will retrun a tuple (output, bias)
        return lm_head_output[0]
...
class LlamaModelPipe(PipelineModule,MegatronModule):
    ...
    def __init__(
        ...
        # Llama use different weight with input embeddings
        # self.specs.append(
        #     TiedLayerSpec('lm_head',
        #                   EmbeddingPipe,
        #                   args.hidden_size,
        #                   args.padded_vocab_size,
        #                   args.hidden_dropout,
