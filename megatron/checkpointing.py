...
def load_checkpoint(model, optimizer, lr_scheduler, load_arg='load', strict=True):
    ...
    if args.deepspeed:
        ...
        load_module_only = True if args.finetune else False
        logger.info(f'load checkpoint:find checkpoint file in load dir={load_dir}, not load optimizer={load_optimizer_states}, \
            load module only={load_module_only}')
        loaded_dir, state_dict = model[0].load_checkpoint(load_dir, load_optimizer_states=load_optimizer_states, load_module_only=load_module_only)
    ...
    # 增加finetune的判断，不对consumed_*_samples进行更新
    ...
    if not release and not args.finetune and not args.no_load_rng:
        try:
            ...
            print_rank_0('Unable to load rng state from checkpoint {}. '
                         ...
                         'exiting ...'.format(checkpoint_name))
            sys.exit()
    #add by yanc to release GPU memory avoid CUDA OOM
