...
from megatron.model.gpt_model_falcon import GPTModelPipe
...
def model_provider(pre_process=True, post_process=True):
    ...
    with deepspeed.zero.Init(data_parallel_group=mpu.get_data_parallel_group(),
                             ...
                             mpu=mpu):
        assert args.deepspeed == True, "args.deepspeed need to be set true when run llama"
        if args.deepspeed:
...
def get_batch_pipe(data):
    ...
    # mask prefix loss for llama, but no prefix attention mask.
    if args.loss_on_targets_only:
        # match `### Response``. only support micro-batch=1. for llama tokenizer.
        for i in range(len(tokens)):
            _partial_prefix_indice = []
            partial_prefix_indice = (tokens[i]==13291).nonzero().squeeze(-1)
            for j in partial_prefix_indice:
                if tokens[i][j-1]==29937:
                    _partial_prefix_indice.append(j.tolist()+2)
                    break
            ...
            # _partial_prefix_indice[0] is the index of first token of target.
